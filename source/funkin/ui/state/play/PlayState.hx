...
class PlayState extends MusicBeatState implements IHook
{
	...
	public override function create()
	{
		...
		// Player goes FIRST so their indexes are first in the array.
				...
				else
				{
					if (DownscrollOption.get())
					{
						if (!dunceNote.isCPUNote)
							dunceNote.y = (playerStrumLineNotes.members[Math.floor(Math.abs(dunceNote.noteData))].y * (startTime - dunceNote.strumTime) * FlxMath.roundDecimal(this.scrollSpeed == 1 ? PlayState.SONG.speed : this.scrollSpeed,
								2))
								- dunceNote.noteYOffset;
						else
							dunceNote.y = (strumLineNotes.members[Math.floor(Math.abs(dunceNote.noteData))].y * (startTime - dunceNote.strumTime) * FlxMath.roundDecimal(this.scrollSpeed == 1 ? PlayState.SONG.speed : this.scrollSpeed,
								2))
								- dunceNote.noteYOffset;
					}
					else
	...
	private function onKeyDown(evt:KeyboardEvent):Void
	{
		...
		{
			// The first X strumline notes are for the opponent! We need to add strumlineSize.
			return (curNote.noteData == keyStrumlineIndex) && (!curNote.isSustainNote);
		});
	...
	function onNoteMiss(currentNote:Note):Void
	{
		...
		else
		{
			...
			replayInputs.push(new ReplayInput(Conductor.songPosition, [currentNote.strumTime, currentNote.noteData, currentNote.sustainLength], true,
				currentNote.rating, currentNote.noteData));
		}
	...
	function onNoteHit(currentNote:Note):Void
	{
		...
		var shouldUseStrumlines = !BotPlayOption.get() || (BotPlayOption.get() && CPUStrumOption.get());
	...
	function startSong():Void
	{
		...
		if (this.modchartActive && PlayState.luaModchart != null)
			PlayState.luaModchart.executeState("songStart", [null]);
		#end
	...
	public function generateSong(songId:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var isEnemyNote:Bool = !NoteUtil.isPlayerNote(newNoteRawData, section.mustHitSection);
	...
	public override function update(elapsed:Float)
	{
		...
		Debug.quickWatch(Conductor.bpm, 'curBPM');
		...
		// Graphics: In beatHit we scale the health icons up, scale them back down linearly.
		this.healthIconPlayer.setGraphicSize(Std.int(FlxMath.lerp(150, this.healthIconPlayer.width, 0.50)));
		this.healthIconPlayer.updateHitbox();
		this.healthIconCPU.setGraphicSize(Std.int(FlxMath.lerp(150, this.healthIconCPU.width, 0.50)));
		...
		if (generatedMusic && !inCutscene /* && songStarted*/)
		{
			...
			songNotes.forEachAlive(function(currentNote:Note)
			{
				...
				if (currentNote.noteData == -1)
				{
					Debug.logWarn('Data for this note is not valid (saw ${currentNote.rawNoteData}), skipping processing.');
					// Cut the note to maintain stability.
					songNotes.remove(currentNote);
					return;
				}
				...
				else
				{
					...
					var strumlineYPos = strumLineNotes.members[currentNote.noteData].y;
					...
					var noteTime = (Conductor.songPosition - currentNote.strumTime) / songMultiplier * this.scrollSpeed;
				...
				else
				{
					...
					{
						if (currentNote.isSustainNote && PlayState.replayActive && findByTime(currentNote.strumTime) != null)
						{
						...
						{
							var notePastStrumline = (Conductor.songPosition - currentNote.strumTime) / songMultiplier > (Scoring.TIMING_WINDOWS[0] * Conductor.timeScale);
		...
		if (this.generatedMusic && this.songStarted && !this.endingSong)
		{
			...
			{
				// If the song is over and there are no notes left in the song, move to end the song in 2 seconds.
		...
		// Cleanup: Make sure the CPU strumlines are positioned properly and stop lighting up once the note was hit.
		cpuStrumLineNotes.forEach(function(spr:StrumlineArrow)
		{
			if (spr.animation.finished)
			{
	...
	{
		Debug.logInfo('The song is over, folks!');
		...
		if (PlayState.replayActive)
		{
			Debug.logInfo('Cleaning up after replay playback.');
		...
		{
			Debug.logInfo('Saving newly generated replay...');
		...
		else
		{
			if (isStoryMode())
			{
				...
				Debug.logInfo('We were in story mode, time to move to the next song!');
				...
				{
					Debug.logInfo('Congratulations, you completed the week! Loading week results.');
	...
	private function popUpScore(curNote:Note):Void
	{
		...
		if (noteJudgement != Shit || noteJudgement != Bad)
		{
			...
			if (PlayState.replayActive)
			{
				var replayInput = findByTime(curNote.strumTime);
				if (replayInput != null)
					msTiming = Util.truncateFloat(replayInput[3], 3);
			}
		...
		if ((CustomControls.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				songNotes.forEachAlive(function(daNote:Note)
				{
					...
					// var correctedNoteData = NoteUtil.getStrumlineIndex();
					if (daNote.canBeHit && !daNote.isCPUNote && !daNote.wasGoodHit && !directionsAccounted[daNote.noteData])
					{
