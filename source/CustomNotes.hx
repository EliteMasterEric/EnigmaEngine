...
class CustomNotes {
  ...
  private static final STRUMLINE_DIR_NAMES: Map<Int, Array<String>> = [
    ...
    9 => ["Left", "Down", "Up", "Right", "Center", "Left Alt", "Down Alt", "Up Alt", "Right Alt"],
  ];
  ...
  public static function loadNoteSprite(instance:FlxSprite, noteStyle:String, noteData:Int, isSustainNote:Bool):Void {
    ...
    switch (noteStyle) {
      case STYLE_PIXEL:
        var widthSize = Std.int(
          PlayState.curStage.startsWith('school')
            ? (instance.width * PlayState.daPixelZoom)
            : (isSustainNote
                ? (instance.width * (PlayState.daPixelZoom - 1.5))
                : (instance.width * PlayState.daPixelZoom)));
  ...
  public static function buildStrumlines(
    isPlayer:Bool,
    yPos:Float,
    strumlineSize:Int = 4,
    ...
    for (i in 0...strumlineDirs.length) {
      ...
      // Create a new note.
      var babyArrow:FlxSprite = new FlxSprite(0, yPos);
      ...
      if (!PlayState.isStoryMode) { 
        babyArrow.y -= 10; 
        FlxTween.tween(
          babyArrow, {y: babyArrow.y + 10, alpha: 1}, 1,
